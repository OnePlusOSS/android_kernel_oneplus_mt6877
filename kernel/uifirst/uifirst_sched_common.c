...
void dequeue_ux_thread(struct rq *rq, struct task_struct *p)
{
	...
	if (!list_empty(&p->ux_entry)) {
		if (atomic64_read(&p->dynamic_ux) &&
			(now - p->dynamic_ux_start) > (u64)ux_max_dynamic_exist * S2NS_T) {
		...
		size_t count, loff_t *ppos)
{
	...
	len = snprintf(buffer, sizeof(buffer),
		"static=%d dynamic=%llx(bi:%d fu:%d rw:%d mu:%d)\n",
		static_ux, atomic64_read(&task->dynamic_ux),
		test_dynamic_ux(task, DYNAMIC_UX_BINDER),
		test_dynamic_ux(task, DYNAMIC_UX_FUTEX),
		test_dynamic_ux(task, DYNAMIC_UX_RWSEM),
		test_dynamic_ux(task, DYNAMIC_UX_MUTEX));
