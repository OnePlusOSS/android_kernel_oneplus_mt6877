			...
			if (insert_idx == ux_sched_cputopo.cls_nr) {
				...
				for (; cls_nr > insert_idx; cls_nr--)
					ux_sched_cputopo.sched_cls[cls_nr] =
						ux_sched_cputopo.sched_cls[cls_nr-1];
...
int task_lb_sched_type(struct task_struct *tsk)
{
	...
	if (test_task_ux(tsk))
		...
		return SA_TOP;
	else if (cgroup_type == SA_CGROUP_FOREGROUD ||
...
const int sched_prio_to_wmult_bg[MAX_ADJ_NICE] = { 5998558, 7146368,
	8421505, 9805861, 11302546, };
...
void pick_ux_thread(struct rq *rq,
...
#define inherit_ux_value(type, value)	\
	((u64)value << inherit_ux_offset_of(type))
...
static bool test_task_exist(struct task_struct *task,
...
inline void inherit_ux_sub(struct task_struct *task,
	int type, int value)
{
	atomic64_sub(inherit_ux_value(type, value), &task->inherit_ux);
}

static void __inherit_ux_dequeue(struct task_struct *task,
	int type, int value)
{
...
void inherit_ux_dequeue_refs(struct task_struct *task,
	int type, int value)
{
	if (!task || type >= INHERIT_UX_MAX) {
		return;
...
static void __inherit_ux_enqueue(struct task_struct *task,
	int type, int depth)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
	unsigned long flags;
	...
	task->ux_depth = task->ux_depth > depth + 1 ?
...
void inherit_ux_enqueue(struct task_struct *task,
	int type, int depth)
{
	if (!task || type >= INHERIT_UX_MAX) {
		return;
...
*/
int set_ux_task_cpu_common_by_prio(struct task_struct *task,
	int *target_cpu, bool boost, bool prefer_idle, unsigned int type)
{
	...
	for_each_cpu(i, cpu_active_mask) {
		...
		/* avoid placing task into high power cpu and
		 * break it's idle state if !prefer_idle */
...
void drop_ux_task_cpus(struct task_struct *p,
...
static inline bool test_sched_assist_ux_type(
		struct task_struct *task, unsigned int sa_ux_type)
{
...
void sched_assist_target_comm(struct task_struct *task)
{
	...
	if (task->pid == task->tgid &&
		(task_uid(task).val == 1000 /* SURFACE_FLINGER_UID */) &&
...
#ifndef CONFIG_OPLUS_SYSTEM_KERNEL_QCOM
	if ((task_uid(task).val == 1000 /* SURFACE_FLINGER_UID */) &&
	...
	if (!strncmp(grp_leader->comm, CAMERA_HAL_SERVER_NAME, TASK_COMM_LEN) &&
		(task_uid(task).val == 1047 /* CAMERASERVER_UID */)) {
		if (task->sched_class == &fair_sched_class) {
...
bool is_task_util_over(struct task_struct *task, int threshold)
{
	...
	if (unlikely(task_index_of_sf_union(task) >= 0)) {
		unsigned long util = sSF_union_util[0] +
			sSF_union_util[1] + sSF_union_util[2];
...
static inline bool is_ux_task_prefer_cpu_for_scene(
		struct task_struct *task, unsigned int cpu)
{
	struct ux_sched_cputopo ux_cputopo = ux_sched_cputopo;
...
bool should_ux_task_skip_cpu(struct task_struct *task,
		unsigned int cpu)
{
...
void set_ux_task_to_prefer_cpu(struct task_struct *task,
		int *orig_target_cpu)
{
	struct rq *rq = NULL;
	struct ux_sched_cputopo ux_cputopo = ux_sched_cputopo;
...
void set_inherit_ux(struct task_struct *task,
		int type, int depth, int inherit_val)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
	unsigned long flags;
#else
	...
	if (task->on_rq && (!rq->curr || (!test_task_ux(rq->curr) &&
		rq->curr->prio > 110))) {
		set_once_ux(task);
		enqueue_ux_thread(task_rq(task), task);
	}
	sched_assist_systrace_pid(task->tgid,
	task->ux_state, "ux_state %d", task->pid);
		...
		struct task_struct *ux_task, int reset_type)
{
	...
	sched_assist_systrace_pid(inherit_task->tgid,
		inherit_task->ux_state, "ux_state %d", inherit_task->pid);

	task_rq_unlock(rq, inherit_task, &flags);
}

void unset_inherit_ux_value(struct task_struct *task,
		int type, int value)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
	unsigned long flags;
	...
	sched_assist_systrace_pid(task->tgid, task->ux_state,
		"ux_state %d", task->pid);
...
u64 sf_union_ux_load(struct task_struct *tsk, u64 timeline)
{
	...
	if (unlikely(index >= 0)) {
		...
		return sSF_union_ux_load[0] +
			sSF_union_ux_load[1] + sSF_union_ux_load[2];
	}
	return timeline;
}
...
#else
	if ((scale_demand(p->ravg.sum) >= sysctl_boost_task_threshold ||
		task_util_mtk(p, true) > sysctl_boost_task_threshold) &&
				cpu < num_mincpu)
#endif
		return true;
#endif
...
void sched_assist_systrace_pid(pid_t pid,
...
static ssize_t proc_ux_state_write(struct file *file,
...
*/
int sysctl_sched_assist_scene_handler(
...
void uxchain_rwsem_wake(struct task_struct *tsk,
...
bool skip_check_preempt_curr(struct rq *rq,
...
bool set_limit_task_target_cpu(struct task_struct *task,
		int *target_cpu)
{
	bool res = false;
		...
		size_t count, loff_t *ppos)
{
	...
	len = snprintf(buffer, sizeof(buffer),
		...
		save_impt_tgid[0], save_impt_tgid[1]);
...
static int __init oplus_sched_assist_init(void)
{
	...
	proc_node = proc_create("sched_impt_task", 0666,
			d_sched_assist, &proc_sched_impt_task_fops);
	if (!proc_node) {
...
void update_task_sched_stat_common(struct task_struct *tsk,
	u64 delta_ns, int stats_type)
{
		...
		if ((tsk->oplus_task_info.sa_info[TST_EXEC][TASK_INFO_SAMPLE-1] != 0) &&
			...
			while (i < TASK_INFO_SAMPLE) {
				if ((tsk->oplus_task_info.sa_info[TST_EXEC][i] < RUNNING_MIN_THREASH) &&
					((tsk->oplus_task_info.sa_info[TST_EXEC][i] * 5) <
					tsk->oplus_task_info.sa_info[TST_SLEEP][i])) {
					fit_small++;
				}
...
void update_sa_task_stats(struct task_struct *tsk,
		u64 delta_ns, int stats_type)
{
	if (!tsk)
		return;
	if (unlikely(!sysctl_sched_assist_enabled))
		return;
	if (tsk->oplus_task_info.im_small && (!is_sched_boost_group(tsk) ||
		!sched_assist_scene(SA_CAMERA))) {
...
void sched_assist_update_record(struct task_struct *p,
		u64 delta_ns, int stats_type)
{
...
void sched_assist_im_systrace_c(struct task_struct *tsk,
		int tst_type)
{
	if (unlikely(!_sched_assist_update_tracemark()))
		return;
