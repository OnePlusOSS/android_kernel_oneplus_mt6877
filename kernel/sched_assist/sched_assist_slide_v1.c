...
void _slide_find_start_cpu(struct root_domain *rd,
...
void slide_calc_boost_load(struct rq *rq,
		unsigned int *flag, int cpu)
{
#ifdef CONFIG_OPLUS_SYSTEM_KERNEL_QCOM
	u64 wallclock = sched_ktime_clock();
#else
...
int sched_frame_rate_handler(struct ctl_table *table,
...
int sysctl_sched_assist_input_boost_ctrl_handler(
	...
	void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	sched_assist_input_boost_duration = jiffies_to_msecs(jiffies) +
		...
		sysctl_sched_assist_ib_duration_coedecay;
