...
static void oplus_show_mem(void)
{
	...
	printk(KERN_INFO "%lu pages HighMem/MovableOnly\n", highmem);
	printk(KERN_INFO "%lu pages reserved\n", reserved);
#ifdef CONFIG_CMA
	printk(KERN_INFO "%lu pages cma reserved\n", totalcma_pages);
#endif
	...
	printk(KERN_INFO "%lu pages hwpoisoned\n", atomic_long_read(&num_poisoned_pages));
#endif
...
/* use two separate locks for heaps and clients in ion_device */
...
static int dump_slab(bool verbose)
{
	if (likely(!verbose)) {
		...
		if (max_cachep || prev_max_cachep) {
			pr_info("name            <active_objs> <num_objs> <objsize> <objperslab> <pagesperslab> : tunables <limit> "
		...
		if (max_cachep) {
			...
			memset(&sinfo, 0, sizeof(sinfo));
			/* TODO maybe we can cache slabinfo to achieve better performance */
...
static void lowmem_dbg_dump(struct work_struct *work)
{
	...
	for (i = MEM_ANON; i < MEM_TOTAL; i++) {
		...
		if (usage <= 0) {
			pr_warn("not support %s stats", text);
			continue;
		}
...
{
	proc_create("lowmem_dbg_test", 0444, NULL, &lowmem_dbg_test_operations);
	return 0;
}
