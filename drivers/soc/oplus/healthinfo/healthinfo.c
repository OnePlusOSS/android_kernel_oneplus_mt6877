...
/******  Sched record  ******/
static inline void ohm_sched_stat_record_common(
...
void ohm_trig_init(void)
{
	...
	ohm_iopanic_mon_trig = (ohm_trig_list & OHM_CTRL_IOPANIC_MON) ?
		true : false;
...
void ohm_logon_init(void)
{
	...
	ohm_cpu_logon = (ohm_logon_list & OHM_CTRL_CPU_CUR) ?
		true : false;
	ohm_memmon_logon = (ohm_logon_list & OHM_CTRL_MEMMON) ?
		true : false;
	ohm_iopanic_mon_logon = (ohm_logon_list & OHM_CTRL_IOPANIC_MON) ?
		true : false;
	ohm_ionmon_logon = (ohm_logon_list & OHM_CTRL_IONMON) ?
		true : false;
...
void ohm_ctrl_init(void)
{
	...
	ohm_cpu_ctrl = (ohm_ctrl_list & OHM_CTRL_CPU_CUR) ?
		true : false;
	ohm_memmon_ctrl = (ohm_ctrl_list & OHM_CTRL_MEMMON) ?
		true : false;
	ohm_iopanic_mon_ctrl = (ohm_ctrl_list & OHM_CTRL_IOPANIC_MON) ?
		true : false;
	ohm_ionmon_ctrl = (ohm_ctrl_list & OHM_CTRL_IONMON) ?
		true : false;
...
/******  Cur cpuloading  ******/
static ssize_t cpu_load_read(struct file *filp,
...
/******  Sched latency stat  *****/
static ssize_t sched_latency_read(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
...
/****** Sched iowait stat  *****/
static ssize_t iowait_read(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
	char page[BUFFER_SIZE_L] = {0};
	int len = 0;
...
static ssize_t fsync_wait_read(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
	char page[BUFFER_SIZE_L] = {0};
	int len = 0;
...
static ssize_t emmcio_read(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
	int len = 0;
	char page[1024] = {0};
...
/****** dstat statistics  ******/
static ssize_t dstate_read(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
	char page[BUFFER_SIZE_L] = {0};
	int len = 0;
...
/******  mem monitor read  ******/
#ifdef CONFIG_OPLUS_MEM_MONITOR
static ssize_t alloc_wait_read(struct file *filp,
	char __user *buff, size_t count, loff_t *off)
{
	char page[1024] = {0};
	int len = 0;
	...
	len = sprintf(page, "total_alloc_wait_h_cnt: %lld\n"
		...
		ohm_memmon_ctrl ? "true":"false", ohm_memmon_logon ?
		"true":"false", ohm_memmon_trig ? "true":"false");
...
static ssize_t ion_wait_read(struct file *filp,
	char __user *buff, size_t count, loff_t *off)
{
	char page[BUFFER_SIZE_L] = {0};
	int len = 0;
...
static ssize_t iowait_hung_read(struct file *filp,
	char __user *buff, size_t count, loff_t *off)
{
...
static ssize_t cpu_info_read(struct file *filp,
	char __user *buff, size_t count, loff_t *off)
{
	char page[BUFFER_SIZE_L] = {0};
	int len = 0;
...
static ssize_t sched_assist_ss_binder_proc_read(
	struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	char page[1024] = {0};
	int len = 0;
...
#ifdef CONFIG_OPLUS_HEALTHINFO_CPUFREQ_MAX
static ssize_t cpufreq_max_read(struct file *filp,
	char __user *buff, size_t count, loff_t *off)
{
	char page[1024] = {0};
	int len = 0;
...
/* thresh read/write */
static ssize_t ohm_thresh_read_common(struct file *filp,
		char __user *buff, size_t count, loff_t *off, int type)
{
	char page[BUFFER_SIZE_S] = {0};
	int len = 0;
...
static ssize_t ohm_thresh_read_iowait(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
...
static ssize_t ohm_thresh_read_fsync(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
...
{
	return ohm_thresh_write_common(file,
		buff, len, ppos, OHM_SCHED_IOWAIT);
}
...
{
	return ohm_thresh_write_common(file,
		buff, len, ppos, OHM_SCHED_SCHEDLATENCY);
}
